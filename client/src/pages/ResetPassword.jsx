...
const ResetPassword = () => {
    ...
    return (
        <div class='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400'>
            ...
            {!isEmailsend &&
                <form onSubmit={onSubmitEmail} class='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm' >
                    ...
                    <div class='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C] '>
                        <img src={assets.mail_icon} alt="" class='w-3 h-3' />
                        <input type="email" placeholder='Email id' class='bg-transparent outline-none text-white' value={email} onChange={(e) => setEmail(e.target.value)} required />
                    </div>
                    <button class='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-3'>Submit</button>
                </form>}
            ...
            {!isOtpSubmitted && isEmailsend &&
                <form onSubmit={onSubmitOtp} class='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'  >
                    ...
                    <button class='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full'>
                        Submit
                    </button>
            ...
            {isOtpSubmitted && isEmailsend &&
                <form onSubmit={onSubmitNewPassword} class='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm' >
                    <h1 class='text-white text-2xl font-semibold text-center mb-4' >
                        New Password
                    </h1>
                    <p class=' text-center mb-6 text-indigo-300'>
                    ...
                    <div class='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C] '>
                        <img src={assets.lock_icon} alt="" class='w-3 h-3' />
                        <input type="password" placeholder='Password' class='bg-transparent outline-none text-white' value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
                    </div>
                    <button class='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-3'>Submit</button>
                </form>}
